...
int New_JPG_filename = 0;
int New_JPG = 0;
char New_JPG_Head[1000] = {0};
int New_JPG_Head_Num = 0;

char *videoname=NULL;

extern unsigned char Frame_NUM_FPGA;
extern int pipe_fdname; 
...
unsigned char SPS_1920x1080[24]={0x00,0x00,0x00,0x01,0x67,0x42,0x40,0x28,
				0xA6,0x80,0x78,0x02,0x27,0xE5,0x40,0x00,
				0x00,0x00,0x01,0x68,0xCE,0X30,0XA4,0X80};

unsigned char SPS_960x544[22]={0x00,0x00,0x00,0x01,0x67,0x42,0x40,0x1F,
				0xA6,0x80,0xF0,0x11,0x64,0x00,
				0x00,0x00,0x01,0x68,0xCE,0X30,0XA4,0X80};

unsigned char SPS_640x512[22]={0x00,0x00,0x00,0x01,0x67,0x42,0x40,0x1E,
				0xA6,0x80,0xA0,0x10,0x64,0x00,
				0x00,0x00,0x01,0x68,0xCE,0X30,0XA4,0X80};

unsigned char SPS_720x576[22]={0x00,0x00,0x00,0x01,0x67,0x42,0x40,0x1F,
				0xA6,0x80,0xB4,0x12,0x64,0x00,
				0x00,0x00,0x01,0x68,0xCE,0X30,0XA4,0X80};

extern EncOutputInfo outinfo;



int	/* write n bytes to a file descriptor */
//fwriten(int fd, void *vptr, size_t n)
fwriten(int fd, char *vptr, size_t n)//g++ :'void*' to 'char*'
{
	int nleft;
	int nwrite;
	char  *ptr;
	...
	//****************************Write to pipe 写给管道***************
		#if 1
			if(outinfo.picType==0)//发送I帧
			{
				if(VPU_Camera_Resolution == 1080)
					write(pipe_fdname, SPS_1920x1080, 24);
				if(VPU_Camera_Resolution == 576)
					write(pipe_fdname, SPS_720x576, 22);
				if(VPU_Camera_Resolution == 544)
					write(pipe_fdname, SPS_960x544, 22);
				if(VPU_Camera_Resolution == 512)
					write(pipe_fdname, SPS_640x512, 22);
			}
			if ((nwrite = write(pipe_fdname, ptr, nleft)) <= 0)
			{
				perror("fwrite: pipe_fdname error !!!\n");
				return (-1);			
			}
		#endif
	//**************************写入文件中*****************************
...
freadn(int fd, char *vptr, size_t n)//g++ :'void*' to 'char*'
{
	int nleft = 0;
	int nread = 0;
	char  *ptr;
...
//2********************************************创建图像.jpg
	...
	{
		if(New_JPG_filename == 0) 		   //第一次创建
		{
			videoname = new_264(2);
			strncpy(cmd->output, videoname , MAX_PATH);
			open_files(cmd);
			//printf("open_files(cmd)");
		}
		
		int fd = cmd->dst_fd;
		fwriten(fd,buf, n);
		#if 0                                  //将头部和图像数据分别写入jpg
			//close_files(cmd);
			if ((cmd->dst_fd > 0)) {
				close(cmd->dst_fd);
				cmd->dst_fd = -1;
				info_msg("Output file \"%s\" closed.\n", cmd->output);
			}
		#else 
			if(New_JPG_filename == 1)		   //第一次写入头部，第二次写图像数据
			{
				close_files(cmd);
				//printf("videoname = %s******* \n",videoname);
			}
			New_JPG_filename ++;
			if(New_JPG_filename == 2)
				New_JPG_filename = 0;
		#endif
	}
	//return (n);
...
//3****************************************发送视频图片****
#if 0
	//if((VPU_Camera_Format == 2)&&(TCP_Accept_VIDEO_fd >= 0))
	if(VPU_Camera_Format == 2)
	{
		if(outinfo.picType==0)//发送I帧
		{
			char* buffer_H264;
			buffer_H264 = (char *)malloc(n+22);
			if(VPU_Camera_Resolution == 544)
				memcpy(buffer_H264,SPS_960x544,22);
			if(VPU_Camera_Resolution == 512)
				memcpy(buffer_H264,SPS_640x512,22);
			memcpy(buffer_H264+22,buf,n);
			int sensor = 0;
			if(VPU_Camera_Resolution == 512)
				sensor = 0;
			else
				sensor = 1;
			Send_H264_Frame(buffer_H264,n+22,sensor);//发送h264帧数据,每次发一帧 
			free(buffer_H264);
			//printf("Send h264 TCP_VIDEO_FB ok *********I Frame*********\n");
		}
		else
		{
			int sensor = 0;
			if(VPU_Camera_Resolution == 512)//红外 channel
				sensor = 0;
			else
				sensor = 1;
			//Send_H264_Frame(buffer,7,1);//发送h264帧数据,每次发一帧   
			Send_H264_Frame(buf,n,sensor);//发送h264帧数据,每次发一帧 
			//printf("Send h264 TCP_VIDEO_FB ok **buf[4] = 0x%02x****************\n",buf[4]);
		}
	}
	//printf("buf[4] = 0x%02x **********************************************\n",buf[4]);
	//if((VPU_Camera_Format == 7)&&(TCP_Accept_VIDEO_fd >= 0))
	if(VPU_Camera_Format == 7)
	{
		if(New_JPG == 0)
		{
			memset(New_JPG_Head,0,sizeof(New_JPG_Head));
			memcpy(New_JPG_Head, buf,n);
			New_JPG_Head_Num = n;
			//printf(" JPG_Head date OK\n wirtedate = %d \n",New_JPG_Head_Num);
		}
		if(New_JPG == 1)
		{
			int sensor = 0;
			if(VPU_Camera_Resolution == 512)
				...
				sensor = 1;
			Send_JPG_Frame(New_JPG_Head, New_JPG_Head_Num, buf, n, Frame_NUM_FPGA, sensor);//发送JPG帧数据,每次发一帧
			//printf("Send JPG TCP_VIDEO_FB ok ********DATA_NUM = %d****New_JPG_Head_Num =%d******\n",n,New_JPG_Head_Num);
		}
		New_JPG ++;
		if(New_JPG == 2)
			New_JPG = 0;
