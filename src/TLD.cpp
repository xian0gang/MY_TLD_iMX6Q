...
int g_idx4;
//Mat img_g;
unsigned char img_g[720*576/MULTIPLE/MULTIPLE];
...
//                    patch = Mat::zeros(grid[i].height, grid[i].width, CV_8UC1);
                    ...
                    ScaleBox srcbox;
                    RectBox dstbox;
                    srcbox.width = 720/MULTIPLE;
                    srcbox.height = 576/MULTIPLE;
                    dstbox.x = grid[i].x;
                    dstbox.y = grid[i].y;
                    dstbox.width = grid[i].width;
                    dstbox.height = grid[i].height;
                    imgRoi(img_g, srcbox, patch, dstbox);

//                    patch = img_g(grid[i]);
                    classifier.getFeatures(patch, grid[i].sidx,ferns, grid[i].width);
...
//        getPattern(patch,nEx[i],dum1,dum2);
        ...
        getPattern(patch, bestbox, nEx[i].data);
    }
    printf("NN: %d\n",(int)nEx.size());
}
...
//    unsigned char img_g[img_glen];
    memset(img_g, 0, 720*576/MULTIPLE/MULTIPLE);
